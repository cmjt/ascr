...
The data included a total of `r nrow(fit$args$capt$bincapt)` calls and `r ncol(fit$args$capt$bincapt)` traps.

The mask used had a `r params$buffer` meter buffer and spacing of `r params$spacing` meters.

```{r plot mask,eval = TRUE,echo = FALSE}
plot.mask(mask,cbind(traps$x,traps$y))
```
...
```{r animation, eval=TRUE,echo=FALSE,fig.show='animate',message=FALSE}
frames <- nrow(fit$args$capt$bincapt)
oopt = ani.options(loop = 1, nmax = frames,ani.width = 800,ani.height = 800)
